...
The MCP server exposes three tools. These mirror a subset of the [Gateway Plugin tools](./plugin-guide.md#tools-reference) with MCP-native input schemas (Zod-based).
...
| Tools | 5 (`vault_execute`, `request_permission`, `vault_await_approval`, `vault_connections_list`, `vault_connection_revoke`) | 3 (`execute`, `list_connections`, `revoke`) |
